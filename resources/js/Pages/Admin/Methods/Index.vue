...
const save = () =>{
    if(operation.value == 1){
        form.post(route('method_payments.store'),{
            onSuccess: () => { ok('Método de pago Creado') }
        });
    }else{
        form.put(route('method_payments.update', id.value),{
            onSuccess: () => { ok('Método de pago Editado') }
        })
    }
};
...
const hanledDelete = (id, name) => {
    ...
    alerta.fire({
        ...
        if (result.isConfirmed) {
            console.log(id);
           formDelete.delete(route('method_payments.destroy', id), {
...
<template>
    <AuthenticatedLayout title="Métodos de pago">
        <template #header>
            <div class="flex flex-wrap justify-between">
                <div>
                    <SectionTitle>
                        <template #title>
                            <div class="text-4xl">
                                Listas de métodos de pago
                            </div>
                        </template>
                        <template #description>
                            En la siguiente lista puede visualizar, editar y eliminar los metodos de pagos creados.
